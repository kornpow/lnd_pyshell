...
def viewhtlcs():
    ...
    async def htlcevents2():
        ws = await websockets.connect(
            "wss://192.168.1.12:8080/v2/router/htlcevents?method=GET",
            ping_timeout=None,
            ping_interval=20,
            ssl=ssl_context,
        ...
        while hi:
            ...
            try:
                ...
                if hi.get("result"):
                    ...
                    fwd_amt = (
                        float(hi["link_fail_event"]["info"]["outgoing_amt_msat"]) / 1000
                    )
...
def closeChannelSync(cid):
    ...
    async def closeChannelStream(cp):
        ...
        ws = await websockets.connect(
            f"wss://192.168.1.12:8080{url}&method=DELETE",
            ping_timeout=None,
            ping_interval=30,
            ssl=ssl_context,
            extra_headers=headers,
...
async def SubscribeChannelEvents():
    ...
    ws = await websockets.connect(
        f"wss://192.168.1.12:8080{url}?method=GET",
        ping_timeout=None,
        ping_interval=30,
        ssl=ssl_context,
        extra_headers=headers,
